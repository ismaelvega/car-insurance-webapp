...
export async function POST(request) {
  try {
    ...
    const userRole = formData.get('userRole'); // Get user role
    ...
    const fileName = `${userIdentifier}_${timestamp}_${file.name}`;
    ...
    if (!processResult.success) {
      // If data processing failed, we might want to delete the uploaded file
...
async function processCSVData(csvText, userEmail, userRole) {
  try {
    ...
    // Find the CLASIFICACION column index
    const clasificacionIndex = headers.findIndex(header => 
      header.toLowerCase().includes('clasificacion') || header.toLowerCase().includes('classification')
    );
    
    if (clasificacionIndex === -1) {
      return { success: false, error: 'CSV must contain a CLASIFICACION column' };
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Get the classification value for this row
      const clasificacion = values[clasificacionIndex];
      
      // BUSINESS RULE: Admin users cannot upload files with ALIANZA classifications
      if (userRole === 'admin' && clasificacion && clasificacion.toLowerCase().includes('alianza')) {
        return { 
          success: false, 
          error: `Los administradores no pueden cargar archivos con clasificación "${clasificacion}". Los datos de Alianza deben ser cargados por usuarios de alianza correspondientes.` 
        };
      }
    ...
    console.log('About to insert records:', records.length);
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      console.log(`Inserting batch ${i + 1} of ${Math.ceil(records.length / batchSize)}`);
